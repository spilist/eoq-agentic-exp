...
1. **테스트 케이스 로드**: `/evaluation/case-{index}/input-text.md` 파일을 읽습니다
   - **CRITICAL**: 반드시 정확히 이 경로의 파일을 읽어야 합니다
   - `{index}`는 호출 시 전달받은 테스트 케이스 인덱스입니다
   - 예: 인덱스 2의 경우 `/evaluation/case-2/input-text.md`를 읽어야 합니다

2. **코치 프롬프트 적용**: `/resources/행동계획-코치-v{version}.md` 파일을 읽어 시스템 컨텍스트로 사용합니다
   - `{version}`은 호출 시 전달받은 코치 프롬프트 버전입니다
   - 예: 버전 0의 경우 `/resources/행동계획-코치-v0.md`를 읽어야 합니다

3. **응답 생성**: 해당 프롬프트 버전으로 작동하는 코치인 것처럼 테스트 케이스에 대한 응답을 생성합니다

4. **원시 응답 반환**: 코치 프롬프트가 지정하는 것 이상의 메타 주석, 평가 또는 포맷팅 없이 생성된 응답을 출력합니다. 결과를 `/evaluation/responses/v{version}-case-{index}.md`에 저장합니다.
...
1. **파일 경로 확인**: `/evaluation/case-{index}/input-text.md` 파일을 읽었는지 확인
   - 읽은 파일 경로가 정확히 index 번호와 일치하는지 확인
   - 예: 인덱스 2라면 case-2/input-text.md를 읽었어야 함
2. **내용 검증**: 읽은 input-text의 내용이 생성된 코치 응답에 반영되었는지 확인
   - 테스트 케이스의 핵심 상황(예: "데이터 분석 팀", "프로젝트 범위 증가" 등)이 응답에 언급되는지 확인
3. **프롬프트 버전 확인**: 올바른 프롬프트 버전으로 작동하고 있는지
4. **응답 적절성**: 응답이 테스트 케이스 시나리오를 다루고 있는지
5. **형식 준수**: 코치 프롬프트의 모든 제약 사항을 따랐는지
6. **순수성**: 출력에 코치 응답만 포함되고 다른 것은 없는지
